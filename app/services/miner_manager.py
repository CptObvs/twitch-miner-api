...
class DockerContainerManager:
    ...
    async def reconcile_all_on_startup(self) -> None:
        """
        On startup: rebuild port map from DB, then verify each running docker container.
        Flips stale instances to STOPPED.
        """
        async with async_session() as db:
            result = await db.execute(
                select(MinerInstance).where(MinerInstance.miner_type == "docker")
            )
        ...
        async with async_session() as db:
            ...
            if changed:
                await db.commit()
                logger.info("Reconciled %d stale docker instance(s) to STOPPED on startup", changed)
    ...
    async def shutdown_all(self) -> None:
        """Stop all running docker containers on app shutdown."""
        async with async_session() as db:
            result = await db.execute(
                select(MinerInstance).where(
                    MinerInstance.miner_type == "docker",
                    MinerInstance.status != InstanceState.STOPPED,
                )
            )
