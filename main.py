...
async def orphan_container_cleanup() -> None:
    while True:
        ...
        await docker_manager.cleanup_orphan_containers()


async def orphan_process_cleanup() -> None:
    while True:
        await asyncio.sleep(300)
        await process_manager.cleanup_orphan_processes()
...
def start_background_tasks(app: FastAPI) -> None:
    ...
    app.state.process_cleanup_task = asyncio.create_task(orphan_process_cleanup())
...
async def stop_background_tasks(app: FastAPI) -> None:
    tasks = []
    for attr in ("cleanup_task", "process_cleanup_task", "memory_gc_task"):
...
async def run_startup(app: FastAPI) -> None:
    ...
    await docker_manager.reconcile_all_on_startup()
    await process_manager.reconcile_all_on_startup()
...
async def run_shutdown(app: FastAPI) -> None:
    ...
    await docker_manager.shutdown_all()
